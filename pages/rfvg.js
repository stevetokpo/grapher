// /rfvg — réglages rFVG par symbole : on calibre les SORTIES (marge du stop, TP,
// break-even) et on enregistre en base ce qui a survécu.
//
// LA PAGE NE DÉCIDE RIEN — elle montre. Le seul chiffre qui compte pour trancher
// est celui de la fenêtre OUT-OF-SAMPLE, celle qui n'a servi à régler aucun
// paramètre. Les colonnes IS et « complet » sont là pour voir l'écart entre les
// deux : un réglage brillant en IS et plat en OOS n'est pas un réglage, c'est un
...
//
// Le formulaire se génère depuis lib/rfvg/params.js, le même schéma qui valide
// côté API : impossible que la page propose un réglage que le simulateur
...
const STATUS = {
  ...
  validated: { label: 'validé',    color: BULL,   hint: "a survécu à l'out-of-sample et au stress des coûts" },
...
export default function RfvgPage() {
  ...
  const [form, setForm] = useState(() => ({
    ...
    fills: 'bar', spreadPoints: 0, split: 0.7, firstN: 0,
  }));
  ...
  const reload = useCallback(async () => {
    const [s, c] = await Promise.all([
      ...
      fetch('/api/rfvg/configs').then(r => r.json()),
    ]);
  ...
  const openConfig = c => {
    ...
    setRuns(null); setProbe(null); setError(null);
  };
  ...
  const post = async (route, payload) => {
    const r = await fetch(`/api/rfvg/${route}`, {
  ...
  const doDelete = async c => {
    setBusy('delete');
    try {
      await fetch(`/api/rfvg/configs?id=${c.id}`, { method: 'DELETE' });
      await reload();
  ...
  return (
    <div className={styles.page}>
      <Head><title>Réglages rFVG — Grapher</title></Head>
      ...
      <header className={styles.header}>
        ...
        <span className={styles.headerTitle}>RÉGLAGES rFVG</span>
          ...
          <div className={styles.symList}>
            ...
            {!configs.length && (
              <p className={styles.empty}>
                ...
                puis enregistre — c'est cette base qui devient la référence des réglages.
              </p>
            )}
          </div>
          ...
          {!form.symbolId ? (
            <section className={styles.card}>
              ...
              <p className={styles.cardSub}>
                La <b>détection</b> est le motif : elle est commune à tous les symboles et ne se
                règle pas ici au petit bonheur — la changer change ce qu'on étudie. Ce qui se
                calibre <b>par instrument</b>, ce sont les <b>sorties</b> : marge du stop, TP,
                break-even. Un TP de 10 points, c'est deux bougies sur XAUUSD et un frisson sur
                Volatility 75.
              </p>
              <p className={styles.cardSub}>
                Ordre de travail : <b>Sonder</b> pour connaître l'échelle réelle du symbole (risque
                structurel médian, nombre de signaux), <b>Simuler</b> pour mesurer, puis
                <b> Enregistrer</b>. Le balayage systématique des grilles se fait en ligne de
                commande — <span className={styles.mono}>node scripts/rfvg-opt.mjs --help</span> —
                parce qu'il produit des centaines de configurations qu'aucun formulaire ne rend
                lisibles.
              </p>
              <p className={styles.caveat}>
                Le chiffre qui décide est celui de la fenêtre <b>out-of-sample</b> : la partie
                récente des données, qui n'a servi à régler aucun paramètre. Tout le reste sert à
                comprendre, pas à conclure.
              </p>
            </section>
          ) : (
            <>
                ...
                <div className={styles.fields} style={{ marginTop: 14 }}>
                  <div className={styles.field}>
                    ...
                    <select className={styles.select} value={form.tf}
                      onChange={e => { setF({ tf: e.target.value }); setRuns(null); setProbe(null); }}>
              ...
              {probe && (
                <section className={styles.card}>
                  <h2 className={styles.cardTitle}>Échelle de l'instrument</h2>
                  <p className={styles.cardSub}>
                  ...
                  <div className={styles.tiles}>
                    <Tile label="Signaux" value={probe.signals.zones}
                      color={probe.signals.zones < 100 ? AMBER : undefined}
                  ...
                  {probe.signals.zones < 100 && (
                    <p className={styles.warn} style={{ marginTop: 12 }}>
                      Moins de 100 signaux sur l'in-sample : régler plus de {Math.floor(probe.signals.zones / 30)} paramètre(s)
                      sur cet échantillon, c'est mémoriser l'historique. Baisser le timeframe,
                      assouplir la détection ou élargir la période avant d'optimiser.
                    </p>
                  )}
                </section>
              )}
              ...
              {runs && (
                <>
                  <section className={styles.card}>
                    ...
                    <p className={styles.cardSub}>
                      Fenêtre {day(windows.oos.from)} → fin des données. C'est le seul bloc qui
                      autorise une conclusion : aucun paramètre n'a été réglé dessus.
                    </p>
                    <div className={styles.tiles}>
              ...
              <section className={styles.card}>
                ...
                <p className={styles.cardSub}>
                  Le stop n'est pas une distance : il est <b>structurel</b>, posé à la clôture de B4
                  sous/sur l'extrême B3-B4. Seule sa <b>marge</b> se règle. Le risque varie donc
                  d'une position à l'autre — c'est pourquoi tout se compte en points, à lot fixe,
                  et pas en R.
                </p>
                <div className={styles.fields}>
                  {EXIT_SCHEMA.map(s => (
                    <Field key={s.key} spec={s} value={form.exit[s.key]}
                      onChange={v => { setE(s.key, v); setRuns(null); }} />
                  ))}
                </div>
              </section>
              ...
              <section className={styles.card}>
                ...
                <p className={styles.cardSub}>
                  Communs à tous les symboles en principe. Les modifier ne calibre pas la stratégie :
                  ça change ce qu'on étudie, et invalide les mesures faites avec les réglages
                  précédents.
                </p>
                <div className={styles.fields}>
                  {DETECT_SCHEMA.map(s => (
                    <Field key={s.key} spec={s} value={form.detect[s.key]}
                      onChange={v => { setD(s.key, v); setRuns(null); setProbe(null); }} />
                  ))}
                </div>
              </section>
