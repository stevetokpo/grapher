// /api/rfvg/configs — les réglages rFVG retenus par symbole (table
// `rfvg_configs`, cf. lib/db.js).
//
...
export default async function handler(req, res) {
  try {
    if (req.method === 'GET') {
      const rows = await query(`
        SELECT c.id, c.symbol_id, s.name AS symbol, c.tf, c.name, c.detect, c.exit_params,
               c.fills, c.spread_pts, c.status, c.metrics, c.notes,
      ...
      return res.json(rows.map(r => ({
        ...
        status: r.status, metrics: parse(r.metrics, null), notes: r.notes,
    ...
    if (req.method === 'POST') {
      ...
      await run(`DELETE FROM rfvg_configs WHERE symbol_id = ? AND tf = ? AND name = ?`,
        symbolId, tf, name);
      await run(`
        INSERT INTO rfvg_configs
          (id, symbol_id, tf, name, detect, exit_params, fills, spread_pts, status, metrics, notes, updated_at)
        VALUES (nextval('seq_rfvg_configs'), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, current_timestamp)
      `, symbolId, tf, name,
    ...
    if (req.method === 'DELETE') {
      ...
      await run(`DELETE FROM rfvg_configs WHERE id = ?`, id);
      return res.json({ ok: true });
    }

    return res.status(405).end();
  } catch (err) {
    console.error('[rfvg/configs]', err);
