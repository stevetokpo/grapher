// POST /api/ko/run — une simulation KO complète, côté serveur.
//
...
//
// Pas de paramètre `engine` ici, contrairement au rFVG : le graphe et le serveur
// appellent le même moteur (lib/signals/engine.js), il n'y a pas de seconde
// implémentation à prendre pour témoin.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const all = simulatePositions(candles, signals, {
      ...ex.params, spreadPts: spread, fills, m1: { bars: m1, ranges },
    });
    ...
    res.json({
      meta: {
        symbolId, tf, fills, pattern: 'ko',
        spreadPoints: spread,
        detect: det.params, exit: ex.params,
        clamped: [...det.clamped, ...ex.clamped],
        from, to, firstN: firstN || null,
        candles: candles.length, m1Bars: m1.length, signals: signals.length,
