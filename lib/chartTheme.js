...
export const DEFAULT_CHART_SETTINGS = {
  ...
  // Graphique linéaire — une seule couleur, la clôture ne connaît pas de sens.
  lineColor:      '#60A5FA',
  lineWidth:      2,
  lineFill:       true,       // dégradé sous la courbe
...
export function resolveChartTheme(raw) {
  ...
  return {
    ...
    // Mode « ligne » : une courbe de clôtures, avec un dégradé optionnel dessous.
    // Le remplissage part de la couleur de la ligne et s'éteint vers le bas ;
    // désactivé, les deux bornes sont transparentes et il ne reste que le trait.
    line: {
      color:  s.lineColor,
      width:  Math.max(1, Math.min(4, s.lineWidth)),
      top:    s.lineFill ? hexToRgba(s.lineColor, bg.dark ? 0.28 : 0.22) : TRANSPARENT,
      bottom: TRANSPARENT,
    },
    volume: {
