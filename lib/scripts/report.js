...
export function summarizeRun(run) {
  ...
  // Le profit se compte sur le PATRIMOINE — solde restant chez le broker PLUS
  // l'argent qui en est sorti —, jamais sur le seul solde : un script qui met
  // ses gains à l'abri afficherait sinon une perte à chaque retrait. Sans
  // mouvement de caisse, `external` vaut zéro et c'est le chiffre d'avant.
  // La protection du solde négatif est REMISE dedans : ce que le broker a
  // effacé, la stratégie l'a bel et bien perdu, et l'ignorer transformerait un
  // compte cramé par un spike en simple mauvaise passe.
  const finalWealth = acc.balance + acc.external;
  const netProfit   = finalWealth - acc.capital;
  const trueProfit  = netProfit - acc.absorbed;
...
export function buildScriptReport({ script, params, accountConfig, run, symbol, tf, notes }) {
  ...
  return {
    ...
    conventions: {
      ...
      drawdown: "calculé sur le PATRIMOINE (solde chez le broker + argent retiré) relevé à chaque bougie, creux traversés en position compris — pas sur la courbe des trades fermés. Un retrait n'est pas une perte",
      caisse: "retireDuCompte / remisSurCompte sont les mouvements décidés par le script : ce qui sort n'est plus exposé — ni au flottant, ni à la marge, ni au stop out. profitNet se lit donc sur le patrimoine, pas sur le solde",
      soldeNegatif: "protection active : une position qui coûte plus que le solde le laisse à ZÉRO. pertesEffacees dit ce que le broker a absorbé, profitNetHorsEffacement ce que la stratégie aurait coûté sans cette protection",
      glissement: "slipPct % du chemin parcouru AU-DELÀ du stop dans la bougie qui l'a touché est payé en plus. À 0, le stop est servi au niveau demandé ; à 100, au pire prix de la bougie",
    ...
    stats: {
      ...
      patrimoineFinal: num(s.finalWealth, 2),
      retireDuCompte: num(s.withdrawals, 2),
      remisSurCompte: num(s.deposits, 2),
      horsDuCompte:   num(s.external, 2),
      pertesEffacees: num(s.absorbed, 2),
      pochesCramees:  s.wipes,
      profitNet:      num(s.netProfit, 2),
      profitNetHorsEffacement: num(s.trueProfit, 2),
