...
//     onRuin({ … même signature })                                     (optionnel)
//     summary({ params, context }) → string                            (optionnel)
//   }
//
// `onRuin` est appelé quand l'équité tombe à zéro, AVANT de déclarer la ruine :
// un script qui garde de l'argent hors du compte (cf. api.deposit) peut y
// recharger et continuer. S'il ne le fait pas, la ruine tombe comme avant.
//
...
//
// LE GLISSEMENT (`slipPct`, cf. account.js) s'ajoute par-dessus. Un stop est
// demandé à un prix et servi au premier prix TRAITÉ au-delà : les deux ne se
// confondent que sur un marché continu. `slipPct` dit quelle part du chemin
// parcouru au-delà du stop DANS cette bougie a réellement été payée — 0 % le
// niveau demandé, 100 % le pire prix de la bougie. Sur un instrument qui bondit
// (Boom, Crash, un gap d'ouverture), c'est la différence entre le stop qu'on
// avait posé et celui qu'on a obtenu.
function exitFill(pos, bar, hit, slipPct = 0) {
  if (hit === 'tp') return pos.tp;
  const frac = Math.max(0, Math.min(100, slipPct)) / 100;
  if (pos.side === 'BUY') {
    const base = Math.min(pos.sl, bar.open);
    return base - frac * Math.max(0, base - bar.low);
  }
  const base = Math.max(pos.sl, bar.open);
  return base + frac * Math.max(0, bar.high - base);
}
...
export function runScript({
  ...
  for (let i = startIdx; i <= endIdx; i++) {
    ...
    for (const pos of [...acc.positions]) {
      ...
      acc.close(pos, exitFill(pos, bar, hit, acc.cfg.slipPct), bar.time, i, reason);
    }
    ...
    acc.mark(bar.close);
    equityCurve.push({ time: bar.time, equity: acc.equity, balance: acc.balance, wealth: acc.wealth });

    const api = makeApi({ acc, bar, i, pending, logs, nextId: () => nextOrder++ });

    if (acc.equity <= 0) {
      // LE COMPTE EST À ZÉRO — mais pas forcément le joueur. Un script qui ne
      // laisse chez le broker que la mise d'une position garde le reste dehors :
      // sa poche est vidée, sa réserve non, et s'arrêter là simulerait une ruine
      // qui n'a pas eu lieu. On lui laisse donc la parole, une fois, avant de
      // trancher : s'il recharge (api.deposit), la partie continue.
      if (script.onRuin) {
        script.onRuin({ candles: bars, bar, i, state, params, account: acc.snapshot(), api, context });
        acc.mark(bar.close);
        equityCurve[equityCurve.length - 1] = { time: bar.time, equity: acc.equity, balance: acc.balance, wealth: acc.wealth };
      }
      if (acc.equity <= 0) {
        acc.ruined   = true;
  ...
  if (closeAtEnd && acc.positions.length > 0) {
    ...
    if (equityCurve.length) {
      equityCurve[equityCurve.length - 1] = {
        time: last.time, equity: acc.equity, balance: acc.balance, wealth: acc.wealth,
      };
    }
  }
...
function makeApi({ acc, bar, i, pending, logs, nextId }) {
  ...
  return {
    ...
    // Caisse — l'argent qui sort du compte sans être perdu, et l'inverse. Ce
    // qui sort n'est plus exposé : ni au flottant, ni à la marge, ni au stop
    // out. C'est là toute la différence entre « j'ai 500 $ » et « j'ai 500 $
    // dont 7,50 chez le broker ». `withdraw` rend ce qui est VRAIMENT sorti,
    // borné à la marge libre.
    withdraw: (usd, reason) => acc.withdraw(usd, { time: bar.time, index: i, reason }),
    deposit:  (usd, reason) => acc.deposit(usd,  { time: bar.time, index: i, reason }),
