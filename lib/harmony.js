...
import {
  HTF_SECONDS, HTF_OFFSET, htfTrendPerBar, htfBarsFromCandles, htfLabel,
  htfSeriesRequest, mergeHtfRequests,
} from './htf';
...
// Ce que les indicateurs TRENDER doivent demander à /api/htf : assez de bougies
// HTF pour enjamber la fenêtre affichée, PLUS les bbLen que la Bollinger consomme
// avant de produire sa première valeur. La forme de la requête (et sa borne sur
// le dernier bucket CLOS) vient de lib/htf.js — le RSIER s'en sert aussi.
export function htfRequests(indicators, candles) {
  ...
  for (const ind of indicators) {
    ...
    for (const key of activeHtfKeys(ind)) reqs.push(htfSeriesRequest(key, candles, bbLen));
  }
