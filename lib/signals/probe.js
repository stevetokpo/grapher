// L'échelle de l'instrument, avant toute optimisation — commun à tous les motifs.
//
...
// utiliser : amplitude médiane par bougie, et surtout la distribution du RISQUE
// STRUCTUREL réel (distance entrée → stop) — c'est elle qui fixe l'échelle utile
// du TP, puisque le seuil de rentabilité dépend du rapport TP / risque.
//
...
import { simulatePositions } from './engine';
import { windowPositions } from './data';
...
/**
 * @param o.symbolId     symbole (pour lire le spread écrit par le broker)
 * @param o.candles      bougies du TF
 * @param o.m1, o.ranges contexte M1 (le simulateur les veut, même en fills 'bar')
 * @param o.signals      signaux détectés (tous, la fenêtre est appliquée ici)
 * @param o.from, o.to   fenêtre, en epoch secondes
 * @param o.slMarginPts  marge du stop à utiliser pour mesurer le risque réel
 */
export async function probeInstrument({ symbolId, candles, m1, ranges, signals,
                                        from = null, to = null, slMarginPts = 2 }) {
  const win = candles.filter(c =>
    (from == null || c.time >= from) && (to == null || c.time < to));
  if (!win.length) throw new Error('aucune bougie dans la fenêtre');
  ...
  // que la mesure porte sur TOUS les signaux, sans qu'un TP serré n'en écarte.
  const probe = simulatePositions(candles, signals, {
    slMarginPts, tpPts: 1e6, fills: 'bar', m1: { bars: m1, ranges },
  });
  const risks = windowPositions(probe, from, to).map(p => p.risk0).sort((a, c) => a - c);

  const inWin = signals.filter(z =>
  ...
  return {
    ...
    signals: {
      count: inWin.length,
      zones: inWin.length,          // nom historique, gardé pour les appelants rFVG
      perDay: +(inWin.length / days).toFixed(2),
      bull: inWin.filter(z => z.side === 'bull').length,
      bear: inWin.filter(z => z.side === 'bear').length,
    },
  };
}
