...
export const EXIT_SCHEMA = [
  ...
  { key: 'dueAfterSl',    label: 'Dû — pertes non remboursées', type: 'int', def: 0, min: 0, max: 100,
    hint: '0 = désactivé. REMBOURSER AVANT DE GAGNER : chaque position clôturée dans le rouge laisse sa perte NETTE sur une ardoise, chaque gain la rembourse en commençant par la plus ancienne. Dès que l’ardoise compte ce nombre de pertes, la position suivante vise le remboursement au lieu de son vrai TP — même s’il tombe plus PRÈS. Le break-even n’est pas touché : il s’arme comme sur n’importe quelle position. ÉVOLUTION : absente de l’EA MT5' },
  { key: 'dueMode',       label: 'Dû — remboursement',        type: 'select', def: 'full', options: ['full', 'step'],
    hint: '« full » = toute l’ardoise d’un coup : l’objectif s’éloigne à mesure qu’elle grossit et peut finir hors d’atteinte. « step » = par bonds de seuil × la perte moyenne encore due, soit la taille de ce qui a armé le dû — on rembourse en plusieurs fois, chacune atteignable' },
];
