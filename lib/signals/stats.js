...
export function computeStats(positions, opts = {}) {
  ...
  // DEUX LECTURES DU MÊME TRADE, et il faut savoir laquelle on regarde.
  //   gainOf   — le résultat du COMPTE, taille de position comprise.
  //   gain1Of  — le même, RAMENÉ À 1 LOT : celui de la stratégie.
  // Tant que le lot vaut 1 — c'est-à-dire partout sauf en escalier — les deux
  // sont le même nombre, et rien de ce qui suit ne change.
  //
  // POURQUOI LES SÉPARER. Tout ce qui JUGE ou RÈGLE la stratégie vit dans
  // l'espace des PRIX : un seuil de rentabilité, un gain moyen, un plafond de
  // stop en points. Les nourrir de résultats pondérés par le lot les réglerait
  // sur le CALENDRIER DES LOTS — quel trade est tombé à quelle marche — au lieu
  // du marché. L'étude du SL plafonné le montrait crûment : elle comparait une
  // perte de compte (−40 à 4 lots) à un plafond de prix (12), et rognait donc
  // n'importe quoi. Ce qui décrit le COMPTE, à l'inverse — total net, courbe
  // cumulée, drawdown —, doit évidemment inclure le lot : c'est ce qu'on gagne.
  const gain1Of = p => p.netPoints1 ?? gainOf(p);
  ...
  // La STRATÉGIE se lit à 1 lot, le COMPTE avec.
  const gains  = resolvedPos.map(gain1Of);
  const gainsC = resolvedPos.map(gainOf);
  const expPts = mean(gains);

  // La courbe et son creux décrivent le COMPTE : le lot y est, forcément.
  let cum = 0;
  ...
  // Le TOTAL, lui, est celui du compte — c'est le seul chiffre de ce bloc qui
  // dise ce qu'on a réellement gagné. Il ne vaut donc pas expPts × n dès que les
  // lots varient, et c'est normal : l'un est l'edge par trade, l'autre la somme.
  const netPts  = gainsC.reduce((s, g) => s + g, 0);
  ...
  // LES DEUX ÉTUDES SE LISENT À 1 LOT, sans exception. Elles comparent des
  // résultats à des seuils EN POINTS DE PRIX (un déclencheur de break-even, un
  // plafond de stop) : mêler la taille de position aux uns quand les autres n'en
  // ont pas ferait choisir le « meilleur » réglage sur le calendrier des lots.
  const sumWinPts  = byStatus.tp.reduce((s, p) => s + gain1Of(p), 0);
  const sumLossPts = byStatus.sl.reduce((s, p) => s - gain1Of(p), 0);
  ...
  if (resolved && tpPts > 0) {
    for (let i = 1; i <= 9; i++) {
      ...
      const spared   = savedPos.reduce((s, p) => s - gain1Of(p), 0);
  ...
  if (resolved && risks.length) {
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      for (const p of byStatus.tp) {
        ...
        else total += gain1Of(p);
      }
      for (const p of byStatus.sl) total -= Math.min(-gain1Of(p), cap);
  ...
  return {
    ...
    // L'objectif RETENU pour cette lecture : celui passé en option quand il est
    // constant, sinon la MÉDIANE de ce que les positions ont réellement visé.
    // Rendu parce qu'un affichage qui reprendrait le réglage brut mentirait dès
    // que l'objectif varie (TP en ATR, dû armé) — et parce que c'est lui, et non
    // le réglage, qui a servi ici au RR médian comme à la grille du break-even.
    tpPts,
