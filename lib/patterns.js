import { isSwingAt, simulatePositions } from './signals/engine';
...
export function calcRFVG(candles, {
  ...
  for (let i = 1; i < n - 1; i++) {
    ...
    const slowBear = slowOpenOnly ? m.open > avgSlow : m.low  > avgSlow;
    const slowBull = slowOpenOnly ? m.open < avgSlow : m.high < avgSlow;
    const maBear = m.low  > avgFast && slowBear;
    const maBull = m.high < avgFast && slowBull;
...
export function calcRFVGPositions(candles, opts = {}) {
  const { slMarginPts = 2, slCapPts = 0, tpPts = 10, beTriggerPts = 0, beLevelPts = 0,
          ...
          uniqueTrade = false, skipAfterTp = 0, spreadPts = 0 } = opts;
  const trades = [];
