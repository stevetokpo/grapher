...
const iso = t => t != null ? new Date(t * 1000).toISOString() : null;
    ...
    const doc = {
      ...
      conventions: {
        unites:  "P&L en POINTS × LOT — c'est le résultat du COMPTE, pas la distance parcourue par le prix. Celle-ci se relit dans sl, tp et risk0, qui restent des prix et ne sont jamais multipliés. Avec lotMode = 'fixe' (le défaut) le lot vaut 1 partout et les deux se confondent, comme depuis toujours. Les champs en R (points / risk0) sont fournis en plus, le risque variant d'une position à l'autre — à lot > 1 ils disent donc bien le R du COMPTE : deux lots à +1 R de prix font +2 R de compte",
                lot:     "lotMode = 'fixe' (défaut) : 1 lot partout, le compte croît linéairement avec l'espérance. lotMode = 'exponentiel' : le lot est MULTIPLIÉ par lotFactor tous les lotStepTrades trades PRIS — les N premiers à 1 lot, les N suivants à F, les N d'après à F², et ainsi de suite. lotMode = 'pas' : le lot est AUGMENTÉ de lotPlus toutes les lotStepTrades — 1, 2, 3, 4… C'est le pendant LINÉAIRE du précédent, et il ne se comporte pas du tout pareil : le poids du dernier bloc y croît comme n et non comme 2^n, si bien que le total reste une moyenne à peu près honnête au lieu d'être dicté par les derniers trades. Des deux escaliers, c'est le seul dont un backtest garde du sens. Ils partagent le compteur de marches (lotStepTrades) et le plafond ; seule la façon de monter d'une marche les sépare. Le compte se fait sur les positions RETENUES dans l'ordre d'ENTRÉE : un ordre jamais servi ('missed') n'est pas un trade et ne fait pas monter la marche ; ses champs lot et tradeNo valent null. lotMax plafonne (0 = aucun), et il n'est pas là par prudence mais par nécessité arithmétique : F^k déborde, et un lot infini rendrait tous les résultats NaN sans qu'une ligne ne le signale. LE LOT NE TOUCHE À RIEN DE LA SIMULATION — entrée, stop, cible et excursions sont des prix et ne bougent pas d'un lot à l'autre ; il multiplie le RÉSULTAT à la toute fin, brut, spread et net ensemble (deux lots paient deux spreads, donc net = brut − spread reste vrai). CE QU'IL FAIT ET CE QU'IL NE FAIT PAS : il multiplie l'espérance de CHAQUE trade — tous les lots étant positifs, le signe de chacun est inchangé —, il n'en crée aucune. MAIS LE TOTAL RÉALISÉ, LUI, PEUT CHANGER DE SIGNE, et c'est là que tout le monde se trompe : ce n'est plus une moyenne mais une somme PONDÉRÉE dont les poids doublent avec le temps, donc dictée par le dernier bloc de trades. Mesuré sur des bougies de test : une configuration qui rend +20 points à lot fixe en rend −240 en escalier, sans qu'une seule entrée ni une seule sortie n'ait bougé. Un backtest en escalier ne mesure donc plus la stratégie, il mesure la chance des derniers trades — à lire À CÔTÉ du résultat à lot fixe, jamais à sa place. Le drawdown max du rapport est en points × lot et grandit avec la série, contrairement au drawdown en pourcentage d'un compte. LE DÛ ignore le lot, exprès : il vise une distance de PRIX, pas un résultat de compte",
      ...
      positions: positions.map(p => ({
        ...
        // La distance RÉELLEMENT visée par CETTE position, toujours — jamais le
        // réglage. Les deux coïncident quand l'objectif est fixe, mais divergent
        // dès qu'il varie : TP réglé en ATR (le mode reste 'points', la distance
        // change à chaque signal), dû qui a pris la place de la cible, ou cible
        // repoussée par le TP dynamique. Reprendre le réglage ferait alors écrire
        // au rapport un objectif que la position n'a jamais eu.
        tpPts:        p.tpDistPts != null ? num(p.tpDistPts)
                      : (params.tpMode === 'points' ? params.tpPts : null),
        // La cible de DÉPART et son éventuelle extension. Absentes des motifs
        // qui n'ont pas de TP dynamique — le champ reste, à null.
        tpBaseDistPts: num(p.tpBaseDistPts),
        tpBoosted:     p.tpBoosted ?? false,
        tpBoostReason: p.tpBoostReason ?? null,
        tpBoostMult:   p.tpBoostMult ?? null,
        tpBoostBars:   p.tpBoostBars ?? null,
        // La SANTÉ de la position, pour les motifs qui en définissent une :
        // est-elle restée saine jusqu'au bout, et combien de bougies l'a-t-elle
        // été. C'est ce qui pilote le BE du malsain et le trade unique 'healthy' ;
        // sans ces deux champs, aucune de ces deux règles ne se relit.
        stayedHealthy: p.stayedHealthy ?? null,
        healthyBars:   p.healthyBars ?? null,
        ...
        // Sans eux, un résultat multiplié par le lot serait illisible : on ne
        // saurait pas si +40 vient d'un gros mouvement ou d'un gros lot.
        lot:          p.lot ?? null,
        tradeNo:      p.tradeNo ?? null,
        profitPoints: num(p.profitPoints),
        // Le même résultat RAMENÉ À 1 LOT : celui de la stratégie, débarrassé du
        // plan de taille. Identique au net tant que le lot vaut 1. C'est lui que
        // lisent le gain moyen, le seuil de rentabilité et les deux études.
        netPoints1:   num(p.netPoints1),
        ...
        // Repères propres au motif, quand il en pose ($$$ : le bord visé, le
        // pivot, la similitude de la figure). Un motif qui n'en a pas ne porte
        // rien de plus, et la forme du document ne bouge pas pour autant.
        ...(p.extra ?? {}),
      })),
    };
