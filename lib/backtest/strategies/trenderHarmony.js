...
const HTF_OFFSET = { W1: 345600 };

// Tendance HTF alignée par bougie chart. Bougies HTF reconstruites par bucket
// de temps (le close d'un bucket est identique quel que soit le TF intermédiaire).
// out[i] = tendance de la bougie HTF précédant celle qui contient candles[i]
// (= dernière HTF clôturée, équivalent Pine request.security(expr[1], lookahead_on)).
function htfTrendPerBar(candles, htfSec, htfOff, bbLen, bbMult) {
  ...
  for (const c of candles) {
    ...
    if (buckets.length === 0 || buckets[buckets.length - 1] !== b) {
      buckets.push(b);
      closes.push(c.close);
    } else {
  ...
  // Tendance de chaque bougie HTF — écart-type population comme Pine ta.stdev
  const trend = new Array(closes.length).fill(0);
  let sum = 0;
  for (let j = 0; j < closes.length; j++) {
    sum += closes[j];
    if (j >= bbLen) sum -= closes[j - bbLen];
    ...
    trend[j] = closes[j] > mean + dev ? 1 : closes[j] < mean - dev ? -1 : 0;
  }
  ...
  for (let i = 0; i < n; i++) {
    ...
    while (j + 1 < buckets.length && buckets[j + 1] <= b) j++;
    out[i] = j >= 1 ? trend[j - 1] : 0;
  }
  return out;
}
