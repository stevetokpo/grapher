...
// Prix de sortie réel. Un SL est rempli au niveau du stop, ou à l'open M1 si le
// marché a ouvert au-delà (gap) — jamais à un prix meilleur que le marché, comme
// le fait déjà le fill des ordres stop d'entrée. Sans cette borne, un stop
// déplacé du MAUVAIS côté du prix (break-even mal réglé) est trivialement
// « touché » et remplirait à un niveau où le marché n'a jamais traité.
// Le TP reste rempli à son niveau : hypothèse conservatrice, on ne s'accorde pas
// le gap en notre faveur.
function exitFill(m1, position, hit) {
  if (hit === 'tp') return position.tp;
  return position.direction === 'BUY'
    ? Math.min(position.sl, m1.open)
    : Math.max(position.sl, m1.open);
}
...
export function runBacktest({ m1Bars, tf, strategy, params, execution = {} }) {
  ...
  const closePosition = (exitPriceRaw, exitTime, exitIndex, exitReason) => {
    ...
    trades.push({
      ...
      sl:          position.sl,        // SL FINAL (un break-even l'a peut-être déplacé)
      tp:          position.tp,
      risk0:       position.risk0,     // distance entrée→SL d'origine — base du R
    ...
    if (pendingStop || (position && (position.sl != null || position.tp != null))) {
      ...
      for (let k = s; k <= e; k++) {
        ...
        if (position && (position.sl != null || position.tp != null)) {
          ...
          if (hit) {
            closePosition(exitFill(m1, position, hit), m1.time, i, hit);
