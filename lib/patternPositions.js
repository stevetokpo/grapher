...
//   REMPLISSAGE DES SORTIES (`exitFill`) — À QUEL PRIX on sort, une fois qu'un
//     niveau est touché. C'est un réglage, pas un détail :
//       'level' (défaut) — au NIVEAU touché, le comportement historique. C'est
//         l'ordre laissé sur le marché : SL et TP sont des ordres réels, servis
//         au prix demandé (au pire de l'ouverture en cas de gap, cf. stopFill).
//       'close'          — À LA CLÔTURE de la bougie qui a touché. Le niveau ne
//         déclenche plus une exécution mais une DÉCISION, appliquée à la fin de
//         la bougie. C'est ce que fait une stratégie qui ne regarde le marché
//         qu'aux clôtures — un EA qui tourne sur `OnBar` plutôt que sur `OnTick`,
//         ou toute règle vérifiée bougie par bougie.
//     CE QUE ÇA CHANGE, ET IL FAUT LE SAVOIR AVANT DE LIRE UN RÉSULTAT :
//       · une position sortie sur TP peut finir PERDANTE, et une sortie sur SL
//         gagnante — la bougie a le temps de revenir sur ses pas ;
//       · le statut continue de dire QUELLE RÈGLE a agi ('tp', 'sl', 'be'), pas
//         si l'issue est gagnante. Le winrate compté sur les statuts n'est donc
//         plus le taux de trades profitables : il devient le taux de trades qui
//         ont ATTEINT leur cible. Les points, eux, restent vrais ;
//       · la perte n'est plus bornée par le risque initial. Une bougie qui
//         traverse le stop et clôture loin en dessous coûte tout l'écart —
//         `maxDrawdownPts` cesse donc d'être plafonné à `risk0`, sans quoi le
//         rapport tairait précisément ce que ce mode ajoute de risque.
...
export function simulatePatternPositions(candles, signals, p, label = 'signal') {
  ...
  for (const { z, eIdx, isBuy, entryIdx, entry, missed } of remplissages) {
    ...
    // À QUEL PRIX ON SORT une fois un niveau touché (cf. `exitFill` en en-tête).
    // En 'close', le niveau ne déclenche plus une exécution mais une DÉCISION,
    // appliquée à la clôture de la bougie qui l'a touché : c'est ce que fait une
    // stratégie qui ne regarde le marché qu'aux clôtures. Le toucher est jugé de
    // la même façon dans les deux modes — seul le prix obtenu change.
    const surCloture = p.exitFill === 'close';
    // Un stop traversé en gap se remplit au pire du niveau et de l'ouverture :
    // jamais mieux que le marché.
    const stopFill = (lvl, k) => surCloture ? k.close
      : isBuy ? Math.min(lvl, k.open) : Math.max(lvl, k.open);
    // Une cible, elle, est servie AU niveau : le prix y est passé pour la
    // toucher. En 'close', même sort que le stop — on attend la fin de la bougie.
    const tpFill   = (lvl, k) => surCloture ? k.close : lvl;
    ...
    for (let j = entryIdx; j < n; j++) {
      ...
      if ((beMalsain || beExist) && !dessus
        ...
        exitIdx = j; exitPrice = tpFill(beLevel, k); exitReason = 'be';
      ...
      if (hitTp(k)) {
        ...
        if (mult > 1) {
          ...
          exitIdx = j; exitPrice = tpFill(tp, k); exitReason = 'tp'; break;
        }
      }
    ...
    // En sortie AU NIVEAU, la bougie du stop est écartée de la MFE : l'ordre
    // entre le sommet et le stop y est inconnu, et le supposer favorable serait
    // optimiste. En sortie À LA CLÔTURE, la question ne se pose plus — la
    // position est restée ouverte toute la bougie, ce sommet a bien été vécu.
    const mfeLast = (!surCloture && (exitReason === 'sl' || exitReason === 'be'))
      ? exitIdx - 1 : exitIdx;
    ...
    //
    // LES DEUX PLAFONDS TOMBENT EN SORTIE À LA CLÔTURE, et ce n'est pas un
    // détail : ils supposent qu'un ordre a été servi AU niveau, donc que la
    // position ne pouvait ni gagner au-delà de sa cible ni perdre au-delà de son
    // risque. En 'close' elle reste ouverte jusqu'à la fin de la bougie — le
    // prix la dépasse pour de bon, dans les deux sens. Les plafonner ici
    // tairait précisément ce que ce mode ajoute de risque.
    if (!surCloture) {
      maxPullupPts   = Math.min(Math.max(0, maxPullupPts),   tpDist);
      maxDrawdownPts = Math.min(Math.max(0, maxDrawdownPts), risk0);
    } else {
      maxPullupPts   = Math.max(0, maxPullupPts);
      maxDrawdownPts = Math.max(0, maxDrawdownPts);
    }
