...
//
// SIMPLE ou EXTRA (réglage `swing`, valable dans les deux modes). Un xFVG EXTRA
// est un motif dont la boîte contient le dernier swing d'en face — swing HAUT
// pour un motif haussier, swing BAS pour un baissier. L'impulsion est allée
// casser cette structure et la zone qu'elle laisse retombe pile dessus : le
// prix qu'on attend est le même des deux points de vue. Le réglage sépare les
// deux familles ('extra' ou 'simple') sans jamais en inventer une troisième —
// 'simple' est la négation exacte de 'extra'.
// LES EXTRAS ONT LEUR PROPRE PATTERN (lib/xfvgx), qui appelle ce détecteur avec
// `swing: 'extra'` et sait, lui, quoi faire du niveau : y poser un ordre en
// attente, avec SL et TP en points. Cette famille-là n'est donc plus une simple
// sélection d'affichage — mais la DÉFINITION reste ici, et elle doit y rester :
// c'est le même motif, et deux définitions divergeraient au premier réglage.
//
// Chaque zone : { side, state: 'active', label, idx, top, bottom, gap,
//                 startTime, endTime, entryIdx, entryTime, entryPrice,
//                 swingIdx, swingTime, swingPrice }
...
//   swing*       → le swing qui a fait l'EXTRA : son indice, son heure, son
//                  niveau. null partout ailleurs — motif simple, ou filtre
//                  éteint. C'est le seul champ que le dessin lit en plus de la
//                  boîte, et c'est aussi le prix à attendre.
...
import { atrArr, smaArr, prevSwingChains } from '../patterns';
import {
...
//             { candles, atr, ma, swings, i, first, m, c, side, isBull, isBear,
//             gap, top, bottom } — et `a` en plus pour le mode 'x3'.
//             `first` est l'indice de la PREMIÈRE bougie de la figure (i−1 en
//             'x3', i en 'x2') : ce que doit connaître une règle qui regarde
//             DERRIÈRE le motif, pour ne pas y inclure le motif lui-même.
...
// ── Le swing précédent ───────────────────────────────────────────────────────
// De quoi répondre à « ce motif retombe-t-il sur la dernière structure d'en
// face ? » sans reparcourir les bougies à chaque candidat.
//
// Le prédicat de swing n'est PAS réécrit ici : c'est isSwingAt, celui de
// l'indicateur SWING et du stop de swing (lib/signals/engine.js, réexporté par
// lib/patterns.js). L'extrême de la bougie pivot doit être STRICTEMENT au-delà
// de ceux des `bars` bougies de chaque côté — un double sommet parfait ne
// confirme donc rien, et c'est voulu : une structure ambiguë n'en est pas une.
//
// Les CHAÎNES elles-mêmes (chains.highs[i] = le dernier swing haut avant i, et
// ainsi de suite en remontant) sont calculées par prevSwingChains, parti dans
// lib/signals/engine.js le jour où le Twins Bars en a eu besoin : deux motifs qui
// parlent du « swing précédent » doivent parler du même.

// Le swing qui fait d'un xFVG un xFVG EXTRA, ou null.
//
// On cherche le swing D'EN FACE — haut pour un motif haussier, bas pour un
// baissier — et on le veut DANS la boîte, bornes comprises. Rien à exiger de
// plus : si le niveau est dans la zone d'un motif haussier, c'est que
// l'impulsion est passée au-dessus (le bas de la 3e bougie est plus haut
// encore), donc qu'elle a cassé cette structure. La cassure est dans la
// géométrie, pas dans une condition de plus.
//
// On part de `first`, la 1re bougie de la figure : les swings formés par le
// motif lui-même ne sont jamais candidats. Et on saute ceux qui n'étaient pas
// encore CONFIRMÉS à la clôture du motif (pivot + bars) — un swing qu'on ne
// connaît pas encore ne peut pas avoir qualifié quoi que ce soit ; il ne compte
// donc pas non plus dans la profondeur.
function extraSwing(x, p) {
  if (!x.swings) return null;         // swingBars à 0 : aucun swing n'existe, donc aucun extra
  const chain = x.isBull ? x.swings.highs : x.swings.lows;
  const last  = x.i + 1;              // la figure se termine là, dans les deux modes
  let seen = 0;
  for (let j = chain[x.first]; j >= 0 && seen < p.swingDepth; j = chain[j]) {
    if (j + p.swingBars > last) continue;
    seen++;
    const price = x.isBull ? x.candles[j].high : x.candles[j].low;
    if (price <= x.top && price >= x.bottom) return { idx: j, price };
  }
  return null;
}

// ── Les règles ───────────────────────────────────────────────────────────────
// Une règle = { id, modes, needsAtr, needsSwings, active(p), test(x, p) }.
...
//   needsSwings pareil, pour les chaînes de swings (prevSwingChains).
//
// Une règle peut poser sur x ce qu'elle a TROUVÉ, quand la zone doit le porter
// ensuite — c'est le cas de 'swing', qui y laisse le niveau à dessiner. C'est le
// seul écart au prédicat pur, et il reste sans effet de bord : x est jeté avec
// le candidat, et calcXFVG ne lit ce qu'on y a posé que si le motif est retenu.
export const RULES = [
  {
    id:       'size',
    modes:    ['x3'],
  ...
  {
    // SIMPLE ou EXTRA. Un xFVG extra est un motif dont la boîte contient le
    // dernier swing d'en face : l'impulsion est allée casser cette structure, et
    // le déséquilibre qu'elle laisse derrière elle retombe pile dessus. Le prix
    // qu'on attend est alors le même des deux points de vue — bord de la zone et
    // ancien sommet (ou creux).
    //
    // Vaut pour les DEUX modes : la question porte sur la zone et sur ce qu'il y
    // a derrière elle, pas sur la façon dont la figure a été dessinée.
    //
    // 'simple' est la NÉGATION exacte de 'extra', pas un autre filtre : les deux
    // familles réunies redonnent toujours le total de 'off'. C'est ce qui permet
    // de les comparer sans se demander qui a mangé les motifs manquants.
    id:          'swing',
    modes:       ['x3', 'x2'],
    needsAtr:    false,
    needsSwings: true,
    active:      p => p.swing === 'extra' || p.swing === 'simple',
    test: (x, p) => {
      const s = extraSwing(x, p);
      x.swing = s;                     // pour la zone : niveau à tracer, cf. calcXFVG
      return p.swing === 'extra' ? s !== null : s === null;
    },
  },
];
...
export function calcXFVG(candles, opts = {}) {
  ...
  const needSwings = rules.some(r => r.needsSwings) && p.swingBars > 0;
  const swings     = needSwings ? prevSwingChains(candles, p.swingBars) : null;

  const ctx = { candles, atr, ma, swings };
  ...
  for (let i = 1; i < n - 1; i++) {
    ...
    zones.push({
      side:      x.side,
      state:     'active',
      // Un motif qualifié par un swing porte son étiquette à lui : la liste
      // affichée n'est jamais ambiguë, même quand on bascule entre les familles.
      label:     x.swing ? 'xFVG+' : 'xFVG',
      idx:       i,
      top:       x.top,
      bottom:    x.bottom,
      gap:       x.gap,
      ...
      // Le swing qui a fait l'extra — null pour un motif simple, et pour tout
      // motif quand le filtre est éteint. La primitive trace le niveau à partir
      // de là ; les autres familles de zones ne le posent pas et ne dessinent
      // donc rien de plus.
      swingIdx:   x.swing ? x.swing.idx : null,
      swingTime:  x.swing ? candles[x.swing.idx].time : null,
      swingPrice: x.swing ? x.swing.price : null,
    });
  }
