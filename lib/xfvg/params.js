...
export const DETECT_DEFAULTS = {
  ...
  // Filtre du SWING PRÉCÉDENT — sépare les xFVG « simples » des « extra ». Un
  // extra est un motif dont la zone contient le dernier swing d'en face : swing
  // HAUT pour un motif haussier, swing BAS pour un baissier. Autrement dit
  // l'impulsion est allée casser cette structure, et la zone laissée derrière
  // elle retombe pile dessus — le prix qu'on attend est donc à la fois le bord
  // du déséquilibre et l'ancien sommet (ou creux).
  //
  // TROIS valeurs dans le DÉTECTEUR, DEUX dans ce formulaire-ci. Les extras sont
  // partis dans leur propre pattern (lib/xfvgx), avec leur entrée, leur gestion
  // et leur rapport : c'est lui qui passe 'extra', et lui seul. Ici il ne reste
  // que 'off' (tous les motifs, extras compris — comme avant) et 'simple' (la
  // négation exacte de 'extra' : tout sauf eux), pour choisir si ce pattern-ci
  // les dessine encore ou les laisse à l'autre. Ne pas retirer 'extra' du
  // détecteur pour autant — c'est la seule définition de la famille, et la
  // dupliquer la ferait diverger.
  swing:      'off',
  swingBars:  2,   // bougies de chaque côté qui confirment un swing (même définition que l'indicateur SWING)
  swingDepth: 1,   // combien de swings en arrière on teste — 1 = le précédent, et lui seul
...
// pilotent rien.
//
// DEUX BLOCS SONT EXPORTÉS À PART, et ce n'est pas de la coquetterie : le pattern
// « xFVG+ » (lib/xfvgx) cherche exactement la MÊME figure — c'est le même
// détecteur, avec `swing` forcé sur 'extra' — et ne s'en distingue que par ce
// qu'il en fait ensuite. Il réutilise donc ces blocs tels quels. Les recopier
// chez lui les aurait fait diverger au premier réglage ajouté ici, et un motif
// réglé au même endroit dans les deux panneaux n'aurait plus donné les mêmes
// zones.
const isX3 = v => v.mode !== 'x2';
const isX2 = v => v.mode === 'x2';

// LA FIGURE et sa boîte : tout ce qui décide qu'un motif existe et où il est
// posé, sans un mot sur ce qu'on en fait.
export const FIGURE_FIELDS = [
  ...
  // ── La boîte, commune aux deux modes ───────────────────────────────────────
...
// CE QUI FAIT UN SWING — la définition du pivot, pas ce qu'on en conclut. Le
// pattern xFVG s'en sert pour écarter les extras, le pattern xFVG+ pour ne
// garder qu'eux : la définition, elle, doit rester la même des deux côtés,
// sinon « le swing précédent » ne désignerait pas le même prix d'un panneau à
// l'autre. Masqué quand `swing` vaut 'off' — aucun swing n'est alors cherché.
export const SWING_CONFIRM_FIELDS = [
  { kind: 'hint', when: v => v.swing !== 'off', text:
    "Le swing cherché est le PRÉCÉDENT, hors du motif : on part de la bougie qui ouvre la figure et "
    + "on remonte. Ceux formés par l'impulsion elle-même ne comptent donc jamais. Un swing n'est "
    + "retenu que s'il était déjà CONFIRMÉ à la clôture du motif — sinon on lirait l'avenir." },

  { kind: 'row', when: v => v.swing !== 'off', fields: [
    { kind: 'number', key: 'swingBars',  label: 'Bougies g/d', min: 1, max: 50, step: 1 },
    { kind: 'number', key: 'swingDepth', label: 'Swings testés', min: 1, max: 20, step: 1 },
  ] },
  { kind: 'hint', when: v => v.swing !== 'off', text:
    "« Bougies g/d » : 2 = un swing confirmé par 2 bougies avant et 2 après, strictement au-delà — "
    + "la définition de l'indicateur SWING et du stop de swing, à l'identique. Plus haut = des "
    + "structures plus rares, donc plus marquées. « Swings testés » : 1 = seul le swing précédent "
    + "compte, la définition stricte. Au-delà, on remonte d'autant de swings et le motif passe si "
    + "l'un d'eux tombe dans la zone — plus permissif, à n'ouvrir qu'en connaissance de cause." },
];

export const FIELDS = [
  ...FIGURE_FIELDS,

  { kind: 'divider', label: 'Swing précédent — les extras' },

  { kind: 'segmented', key: 'swing', label: 'Motifs gardés', options: [
    { value: 'off',    label: 'Tous' },
    { value: 'simple', label: 'Sans les extras' },
  ] },
  { kind: 'hint', text:
    "Un xFVG EXTRA est un motif dont la zone contient le dernier swing d'en face : swing HAUT pour "
    + "un motif haussier, swing BAS pour un baissier. L'impulsion est allée casser cette structure, "
    + "et la zone qu'elle laisse retombe pile dessus — le prix qu'on attend est donc à la fois le "
    + "bord du déséquilibre et l'ancien sommet (ou creux)." },
  { kind: 'hint', text:
    "CES MOTIFS-LÀ ONT DÉSORMAIS LEUR PROPRE PATTERN, « xFVG+ » dans la liste : c'est lui qui les "
    + "détecte, les joue et en rend compte. Il ne reste ici qu'à choisir si on les affiche encore. "
    + "« Tous » = comme avant, extras compris — si l'autre pattern est allumé, la même boîte sera "
    + "dessinée deux fois. « Sans les extras » les lui laisse : c'est la négation EXACTE de sa "
    + "sélection, les deux réunis font toujours le total de « Tous »." },
