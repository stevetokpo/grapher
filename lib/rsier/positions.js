...
//
// LE SENS JOUÉ (`tradeSide`) est un réglage, pas un choix caché, et son défaut
// est la CONTINUATION : on VEND la survente et on ACHÈTE le surachat, un RSI HTF
// en surzone étant lu comme une tendance en cours plutôt que comme un excès à
// contrer. 'reversion' fait exactement l'inverse. Le SENS DE LA ZONE, lui, ne
// bouge jamais — une bande verte reste une survente, et c'est elle qu'on vend.

import { calcRsier } from './detect';
import { DETECT_DEFAULTS, POSITION_DEFAULTS, playedSide } from './params';
...
export function calcRsierPositions(candles, opts = {}, htfSeries = null) {
  ...
  for (const z of zones) {
    ...
    signals.push({
      // Le MÊME `playedSide` que le graphe : une seule bascule surzone → sens
      // joué pour les deux, donc un repère qui ne peut pas annoncer l'inverse
      // de la position prise sous lui.
      side:    playedSide(z.side, p),
      fromIdx,
      toIdx,
      ...
      level:   null,
      // Repères de lecture du signal. Le simulateur ne recopie dans ses
      // positions que les champs qu'il connaît (level, risque, sorties…) : ces
      // trois-là ne vont pas plus loin. La surzone d'où vient une position se
      // relit donc par `tradeSide` — sens joué inversé par rapport à la zone en
      // continuation, identique en réversion.
      zoneSide: z.side,
      rsiStart: z.rsiStart,
      htf:      z.htf,
    });
