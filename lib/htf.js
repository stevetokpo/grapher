...
// Bougies HTF COMPLÈTES (OHLC) reconstruites depuis les bougies du graphe.
//
// La fonction ci-dessus ne rend que la clôture, ce qui suffit à une moyenne ou à
// un RSI. Un motif qui lit des mèches et des corps — le $$$ et ses gaps — a
// besoin des quatre prix, d'où cette variante. Même arithmétique de bucket, pour
// que les deux découpent le temps exactement pareil.
//
// LA DERNIÈRE BOUGIE EST ÉCARTÉE TANT QU'ELLE N'EST PAS CLÔTURÉE. Sans ça, un
// motif détecté sur un bucket en cours se déformerait à chaque tick puis
// disparaîtrait — le repaint dans sa forme la plus visible. Le pas du graphe est
// déduit des deux dernières bougies : le bucket est complet quand la dernière
// bougie qu'il peut contenir est arrivée.
export function htfOhlcFromCandles(candles, htfSec, htfOff = 0) {
  ...
  const pas = n > 1 ? candles[n - 1].time - candles[n - 2].time : 0;
  const dernier = out[out.length - 1];
  if (dernier && candles[n - 1].time + pas < dernier.time + htfSec) out.pop();
...
// sans arrêt).
//
// La borne `from` couvre la PREMIÈRE bougie du graphe, moins le préchauffage.
// C'est elle qui décide de la taille de la série, et rien d'autre : la fenêtre
// affichée est intégralement couverte, quelle que soit sa profondeur. Un plafond
// fixe sur le nombre de bougies HTF, lui, tronquait la série par son côté ANCIEN
// dès que le rapport TF-du-graphe / HTF devenait grand (un M5 lu depuis un M1
// chargé sur 200 000 bougies : ~40 000 buckets, plafonnés à 5 000 = 17 jours
// couverts sur 5 mois). Les bougies non couvertes ne portaient alors aucune
// valeur HTF, donc aucune zone, SANS que rien ne le signale — le préchauffage
// était satisfait, il ne regarde que la longueur de la série.
//
// Renvoie { key, sec, off, from, to, limit } — ou null si le HTF est inconnu.
export function htfSeriesRequest(key, candles, warmup) {
  ...
  // Début du bucket de la première bougie, reculé de `warmup` + 2 buckets : de
  // quoi que le calcul (Bollinger, RSI…) ait déjà produit une valeur quand la
  // première bougie du graphe arrive.
  const firstBucket = Math.floor((first - off) / sec) * sec + off;

  return {
    key, sec, off,
    from:  firstBucket - (warmup + 2) * sec,
...
//
// « Le plus d'historique » se juge sur `from` — la borne qui décide vraiment de
// ce qui est servi ; `limit` suit, pour que la requête retenue reste cohérente
// avec sa propre borne.
export function mergeHtfRequests(...lists) {
  ...
  for (const list of lists) {
    for (const r of list ?? []) {
      ...
      if (!prev || r.from < prev.from) byKey.set(r.key, r);
    }
  }
  return [...byKey.values()];
}
