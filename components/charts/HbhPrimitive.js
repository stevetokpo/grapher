// Lightweight-charts series primitive that paints HBH / BHB pattern zones as
// translucent rectangles. Each zone is the high–low range of the pattern's
// middle candle (teal = HBH bullish, red = BHB bearish), with an optional
// dashed median line. Like FvgPrimitive, it recomputes its pixel coordinates on
// every chart redraw, so the boxes stay glued to the candles through pan / zoom.
//
// Usage:
//   const p = createHbhPrimitive();
//   candleSeries.attachPrimitive(p);
//   p.update(zones, { bullColor, bearColor, opacity, showMid, showLabel });
  ...
  draw(target) {
    ...
    target.useBitmapCoordinateSpace(scope => {
      ...
      for (const r of data.rects) {
        ...
        // median line (H/2) — dashed
        if (data.showMid && r.yMid != null) {
          const yMid = Math.round(r.yMid * vr);
          ctx.setLineDash([Math.round(4 * hr), Math.round(3 * hr)]);
          ctx.beginPath();
          ctx.moveTo(x1, yMid + 0.5); ctx.lineTo(x2, yMid + 0.5);
          ctx.stroke();
          ctx.setLineDash([]);
        }
...
class HbhPaneView {
  constructor(source) {
    this._source = source;
    this._renderer = new HbhRenderer();
  }
  ...
  update() {
    ...
    for (const z of zones) {
      ...
      const yMid = z.mid != null ? series.priceToCoordinate(z.mid) : null;

      rects.push({
        x1, x2, extendRight,
        yTop, yBottom, yMid,
        color: z.side === 'bull' ? opts.bullColor : opts.bearColor,
        label: z.side === 'bull' ? 'HBH' : 'BHB',
      });
    }
...
class HbhPrimitive {
  constructor() {
    ...
    this._zones = [];
    this._opts = { bullColor: '#26A69A', bearColor: '#EF5350', opacity: 0.18, showMid: true, showLabel: true };
    this._paneViews = [new HbhPaneView(this)];
  }
...
export function createHbhPrimitive() {
  return new HbhPrimitive();
}
